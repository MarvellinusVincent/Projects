...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new = [p for p in paragraphs if select(p) == True]
    if k + 1 > len(new):
        return ''
    return new[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def select(paragraphs):
        new_paragraph = remove_punctuation(paragraphs).lower().split()
        for i in topic:
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    smallest_word = min(valid_words, key = lambda valid_words: diff_function(typed_word, valid_words, limit))
    if diff_function(typed_word, smallest_word, limit) > limit:
        return typed_word
...
def feline_flips(start, goal, limit):
    ...
    # BEGIN PROBLEM 6
    """
    Don't really understand the first 2 lines of code
    """
    def helper(start, goal, length):
        if length > limit:
            return limit + 1
        s, g = len(start), len(goal)
        if s == 0 and g == 0:
            return length
        elif s == g:
            if start[0] == goal[0]:
            ...
            return helper(start, goal[:s], length + g - s)
        return helper(start[:g], goal, length + s - g)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        list.append([player[i] - player[i-1] for i in range (1,len(player))])
...
def fastest_words(match):
    ...
    for word in word_indices:
        ...
        for player in player_indices:
            if time(match, player, word) < min_num:
                i = player
                min_num = time(match, player, word)
        fastest[i].append(word_at(match, word))
    return fastest
    # END PROBLEM 10
